...
class TajweedRuleOfNunSakenSeeder extends Seeder
{
    public function run(): void
    {
        $path = database_path('data/1_role_neen_saken.json');

        if (!File::exists($path)) {
            $this->command->error('1_role_neen_saken.json file not found in database/data/');
            return;
        }
        ...
        if (!is_array($segments) || empty($segments)) {
            $this->command->error('1_role_neen_saken.json is empty or invalid.');
            return;
        }
        ...
        $this->command->info('Tajweed rule of nun saken relations seeded successfully.');
    }
}
