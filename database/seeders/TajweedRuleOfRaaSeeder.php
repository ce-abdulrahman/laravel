...
class TajweedRuleOfRaaSeeder extends Seeder
{
    public function run(): void
    {
        $path = database_path('data/4_raa_tajweed.json');

        if (!File::exists($path)) {
            $this->command->error('4_raa_tajweed.json file not found in database/data/');
            return;
        } 
        ...
        if (!is_array($segments) || empty($segments)) {
            $this->command->error('4_raa_tajweed.json is empty or invalid.');
            return;
        }
        ...
        $this->command->info('Tajweed rule of Raa relations seeded successfully.');
    }
}
