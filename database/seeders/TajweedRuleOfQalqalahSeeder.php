...
class TajweedRuleOfQalqalahSeeder extends Seeder
{
    public function run(): void
    {
        $path = database_path('data/5_qalqala_tajweed_results.json');

        if (!File::exists($path)) {
            $this->command->error('5_qalqala_tajweed_results.json file not found in database/data/');
            return;
        }
        ...
        if (!is_array($segments) || empty($segments)) {
            $this->command->error('5_qalqala_tajweed_results.json is empty or invalid.');
            return;
        }
        ...
        $this->command->info('Tajweed rule of Qalqalah relations seeded successfully.');
    }
}
