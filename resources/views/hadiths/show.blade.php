...
<div class="quran-dashboard">
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-8">
                ...
                <div class="quran-card-body pt-0">
                    ...
                    <div class="p-3 bg-light rounded-3 mb-3 border-start border-primary border-3">
                        <div class="d-flex align-items-center mb-2">
                            <span class="badge bg-primary-subtle text-primary-emphasis me-2">KU</span>
                            <h6 class="mb-0 fw-bold small text-muted">{{ __('hadiths.fields.translation_ku') }}</h6>
                        </div>
                        <div class="lead-text" style="font-size: 1.1rem; line-height: 1.6; color: var(--quran-text-primary);">
                            {{ $hadith->translation_ku }}
                        </div>
                    </div>

                    <!-- English Translation -->
                    @if($hadith->translation_en)
                        <div class="p-3 bg-light rounded-3 border-start border-secondary border-3">
                            <div class="d-flex align-items-center mb-2">
                                <span class="badge bg-secondary-subtle text-secondary-emphasis me-2">EN</span>
                                <h6 class="mb-0 fw-bold small text-muted">{{ __('hadiths.fields.translation_en') }}</h6>
                            </div>
                            <div class="lead-text" style="font-size: 1.05rem; line-height: 1.6; color: var(--quran-text-primary);">
                                {{ $hadith->translation_en }}
                            </div>
                        </div>
                    @else
                        <div class="text-center p-3 text-muted border border-dashed rounded-3">
                            {{ __('hadiths.fields.translation_en') }}: {{ __('common.none' ?? 'None') }}
                        </div>
                    @endif
                </div>
            </div>
            ...
            <!-- Explanations Card -->
            @if($hadith->explanation_ku || $hadith->explanation_en)
                <div class="quran-card">
                    <div class="quran-card-header border-bottom-0">
                        <h5 class="quran-card-title mb-0">
                            <i class="bi bi-info-circle-fill me-2 text-info"></i>
                            {{ __('hadiths.sections.explanations') }}
                        </h5>
                    </div>
                    <div class="quran-card-body pt-0">
                        @if($hadith->explanation_ku)
                            <div class="mb-4">
                                <h6 class="fw-bold text-info mb-2">
                                    <i class="bi bi-chat-left-text me-1"></i>
                                    {{ __('hadiths.fields.explanation_ku') }}
                                </h6>
                                <p class="text-muted mb-0" style="white-space: pre-line; line-height: 1.6;">
                                    {{ $hadith->explanation_ku }}
                                </p>
                            </div>
                        @endif

                        @if($hadith->explanation_en)
                            <div>
                                <h6 class="fw-bold text-info mb-2">
                                    <i class="bi bi-chat-left-text me-1"></i>
